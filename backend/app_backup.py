    ...
    try:
        ...
        for item in data.get("results", []):
            incident_data = {
                ...
                "latitude": item.get("location", {}).get("shape", {}).get("links", [{}])[0].get("points", [{}])[0].get("lat", 0),
                "longitude": item.get("location", {}).get("shape", {}).get("links", [{}])[0].get("points", [{}])[0].get("lng", 0),
                "severity": item.get("criticality", {}).get("severity", 0),
                "criticality": item.get("criticality", {}).get("description", "minor"),
                "start_time": item.get("startTime", datetime.utcnow().isoformat()),
                "end_time": item.get("endTime"),
                "road_name": item.get("location", {}).get("roadName", {}).get("value"),
                "length": item.get("location", {}).get("length", 0)
            }
...
async def get_analytics_summary():
    ...
    filters = {
        "start_time": f"gte.{(datetime.utcnow() - timedelta(hours=24)).isoformat()}"
    }
